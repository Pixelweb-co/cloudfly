...
class CallSession:
    ...
    def handle_dtmf(self, digit):
        ...
        self.stop_current_playback() # Barge-in with DTMF
...
class ARIBot:
    ...
    def on_message(self, ws, message):
        try:
            ...
            if etype not in ['DeviceStateChanged', 'ChannelVarset']: # Filter noisy events
    ...
    def handle_dtmf_event(self, event):
        ...
        digit = event['digit']
        print(f"🎹 DTMF {digit} on channel {cid}")
        if cid in self.sessions:
            self.sessions[cid].handle_dtmf(digit)
        else:
            # If not found, try to find the session this channel belongs to
            # (In a bridge, the caller might be a different CID)
            for session in self.sessions.values():
                # We could check bridges here, but for now let's just log it
                pass
