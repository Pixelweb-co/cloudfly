...
import type { QuoteType } from '@/types/ventas/quoteTypes'
...
const QuotesListTable = ({ tableData, onReload }: { tableData: QuoteType[], onReload: () => void }) => {
                ...
                cell: ({ row }) => (
                    <div className={classnames('flex items-center gap-2')}>
                         <i className={classnames('tabler-circle-filled text-[10px]', {
                            'text-secondary': row.original.status === 'DRAFT',
                            'text-info': row.original.status === 'SENT',
                            'text-success': row.original.status === 'ACCEPTED',
                            'text-error': row.original.status === 'REJECTED',
                            'text-warning': row.original.status === 'EXPIRED',
                         })} />
