...
class AutonomousMarketingWorker:
    ...
    def analyze_company_and_products(self, company_name: str, biz_type: str, biz_desc: str, products: List[Dict]) -> Dict:
        ...
        system_prompt = "Eres un analista de marketing experto en B2B y neuromarketing. Responde estrictamente con un objeto JSON."
...
2. Si es B2B, genera una lista de hasta 4 categorías específicas de clientes ideales, si ya existen busca otras (ICP / Nichos) en español que se puedan buscar en Google Maps (ej: "restaurantes", "hoteles", "panaderías", "consultorios médicos").
    ...
    def execute_workflow(self):
        ...
        for idx, company in enumerate(companies, 1):
            ...
            for cat_idx, category in enumerate(analysis.get("categories", []), 1):
                ...
                # Extract location from company/tenant database columns (country only for now)
                country = company.get("pais_nombre") or "Colombia"
                state = None
                city = None

                # Fetch leads via Prospector with dynamic location (country only for now)
                logger.info(f"      → Prospecting leads for '{category}' in {country} (limit=5)...")
